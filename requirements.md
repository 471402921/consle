...
1. **room_id 怎么生成 / 分发?** 开发期 hardcode 一个就行;生产期是不是给 app 嵌入一个二维码 + 时效 token,让 console 扫码加入?
   - ✅ **MVP (2026-05-17)**:hardcode `cute-mvp-2026-05-17-jet-dev-aaaaaaaaaaaaaa`(40 字符,可读,含 owner + 日期方便 debug)。两端 env / console UI 共用同一个值。
   - 生产期(post-MVP):待定。二维码 + 时效 token 是预期方向,届时再做。

2. **多 app 同时连同一 room?** 当前协议假设 1 app + 1 console 一对一,如果允许多 app(比如设计师 + 开发者两台手机同一场景),需要 room 内 role 进一步区分
   - ✅ **MVP**:relay 不强制限制(实现是 `Set<Member>`),但**约定 1 app + 1 console**,开发期靠纪律。多 app 时 console 发的命令会被广播给所有 app,行为不可预期。
   - 未来:若真有多 app 需求,在 envelope 加 `sender_id` 字段,接收端按 id filter。

3. **离线 fallback 行为?** app 启动期连不上 relay 时已经自动走 autonomous(不阻塞 demo),需要给一个**用户可见的提示**吗?(toast / indicator 红色?)还是 silent?
   - ✅ **MVP**:console 端状态灯已经够("已失败(5 次重连用尽)"红点 + 文字)。
   - RN app 端:推荐在 `ConnectionIndicator` 同样以红色 + 文案"console 连接失败,角色走自治"提示。**toast 可选,不强制**。具体看 [handoff/cute_pixel.md §6](handoff/cute_pixel.md)。

4. **proto 扩展会影响 ADR-007**:CHARACTER_* messages 让 proto 从"场景级"扩到"实体级",需要 ADR-007 修订一条"entity scoping"约束(避免未来 messages 数量爆炸)
   - ✅ **建议**:实体级 message 用 `<ENTITY>_<VERB>` 命名(如 `CHARACTER_SET_VELOCITY`),payload 内部不再嵌实体类型。ADR-007 修订归 cute_pixel 那边推。已写进 [handoff/cute_pixel.md §1](handoff/cute_pixel.md)。

5. **console 观察模式?** 一个权限只看不发的 role(`role=observer`),给设计师同事围观开发者操作。MVP 不做,但 protocol 层是否预留?
   - ✅ **MVP 不做,protocol 层也不预留**。YAGNI。relay 加一个 `role=observer` if 分支几行代码,等真有人要再加,不为不存在的需求设计。

6. **录像 / replay?** 把 console 发出的 commands 流录下来重放给同一 app,做 demo 测试。MVP 不做,但 envelope 加 `ts` 字段就是为这件事留口子。
   - ✅ **MVP 不做。envelope `ts` 字段已经为此留口子**;真要做,dump envelope log,replay 时按 ts 节奏发即可。
...
| 2026-05-17 | §10 open questions 全部 resolved(MVP 方案);scaffold + 部署上线 | Jet + Claude |
| 2026-05-17 | 域名 + 正经 TLS 上线(`console.ewow.cn`,腾讯免费 DV 90 天);asset-lab 让位 443 走 nginx 反代 | Jet + Claude |
| 2026-05-17 | `:443` 被腾讯 anti-scan 拦截(未备案子域 + 高频 TLS),主流量切到 `console.ewow.cn:18789`(非标端口绕过),URL 变了 | Jet + Claude |
